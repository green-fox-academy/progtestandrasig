...
public class StudentCounter {
    public static void main(String... args) {
        ...
        List<String> minFourCandies = new ArrayList<>();
        double sumOfAgesHasLoserStudents = 0;

        for (Map row : listOfMaps) {

            if ((int)row.get("candies") > 4)
                minFourCandies.add(row.get("name").toString());
            else sumOfAgesHasLoserStudents += Double.parseDouble(row.get("age").toString());
        }

        System.out.println("Min. four candies..: " + String.join(", ", minFourCandies));
        System.out.println("Sum of ages has loser students..: " + sumOfAgesHasLoserStudents);
    }
}